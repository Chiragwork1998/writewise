...
CONFIG: dict[str, Any] = {
    ...
    # A chapter the primary model cannot deliver (the provider closes long streams under load;
    # the richest chapter is the likeliest) is retried ONCE on this model before being given
    # up. Measured: flash writes 23 items / 16 on-target vs pro's 26 / 20 -- a thinner
    # chapter beats an empty one. Set to "" to disable.
...
def generate(profile: dict[str, Any], evidence: dict[str, list[dict[str, Any]]],
    ...
    with httpx.Client(base_url=api_base, headers=headers,
        ...
        def one_category(code: str, name: str, model_name: str | None = None
            ...
            data = run_with_repair(
                client, use_model, code, system, user,
        ...
        def safe(code: str, name: str) -> tuple[str, list[dict[str, Any]] | None]:
            try:
                ...
                fallback = str(CONFIG.get("fallback_model") or "")
                if fallback and fallback != model:
                    log(f"[{code}] {model} failed ({str(exc)[:90]}); retrying once on {fallback}")
                    try:
                        return one_category(code, name, fallback)
